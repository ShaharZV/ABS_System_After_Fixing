...
describe("User Data Table Tests", () => {
  // Check if "חפש משתמש" input is on screen
  test("renders search input ", () => {
    // Arrange
    render(<UserDataDT setDidDataChanged={dummyFunctionForTests} />);

    // Assert
    const desiredElement = screen.getByPlaceholderText("חפש משתמש");
  ...
  test("renders new object form if clicked on add new button", () => {
    ...
    const buttonElement = screen.getByText("צור משתמש חדש");
    userEvent.click(buttonElement);

    // Assert - check if form of adding new object is open now
    const desiredElement = screen.getByTestId("createNewUserForm");
    expect(desiredElement).toBeInTheDocument();
  });
  ...
  test("renders new object form if clicked on add new button", () => {
    ...
    const closeButton = screen.getByTestId("closeNewUserFormButton");
    ...
    const desiredElement = screen.queryByTestId("createNewUserForm");
